...
namespace RW_Herzblatt.Detour
{
    ...
    internal static class _ChildRelationUtility
    {
        ...
        [Detour(typeof(ChildRelationUtility), bindingFlags = (BindingFlags.Static | BindingFlags.Public))]
        internal static float ChanceOfBecomingChildOf(Pawn child, Pawn father, Pawn mother, PawnGenerationRequest? childGenerationRequest, PawnGenerationRequest? fatherGenerationRequest, PawnGenerationRequest? motherGenerationRequest)
        {
            ...
            if (father != null && child.GetFather() == null)
            {
                ...
                if (father.story.traits.HasTrait(TraitDefOfHerzblatt.Bisexual))
                {
                    num4 = 0.6f;
                }
            }
